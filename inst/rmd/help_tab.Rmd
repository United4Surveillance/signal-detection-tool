...
Here you can set the p-value which is used by several signal detection algorithms to determine the alert threshold. The p-value determines how sensitive the algorithm is to changes in the data. Lower p-values result in higher thresholds, meaning fewer alerts and a less sensitive algorithm. Higher p-values lower the threshold, making the algorithm more sensitive and increasing the likelihood of detecting signals. It is possible to apply the correction to the following algorithms: FarringtonFlexible, Mean, Timetrend, Harmonic, Harmonic with timetrend, Step harmonic, Step harmonic with timetrend. 
