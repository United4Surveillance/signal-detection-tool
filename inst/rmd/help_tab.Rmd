...
This app requires specific input data formatting, which is described in detail in the description, which is described in detail in the README (see the Data section).
...
By default the selected time period ranges from the first date of the linelist to its end date. Here you can choose to use a different end date.
...
The "Show expectation and threshold for historic data" feature, accessible via an additional button, extends the expectation and/or threshold to historic data. The feature can only be used with FarringtonFlexible, EARS and CUSUM.
...
The `Signal Line List` tab displays all signals in a line list. Here you can select a signal for further investigation. The cases associated with the selected signal are then shown in a separate line list.
