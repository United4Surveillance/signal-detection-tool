---
title: "`r params$disease` - `r params$country`"
subtitle: "Signal Detection Report"
date: "Report generated on `r lubridate::stamp('28 April 2022', locale = ifelse(grepl('en_US.UTF-8', Sys.getlocale(), ignore.case = TRUE), 'en_US.UTF-8', Sys.getlocale('LC_TIME')), orders = 'dmy')(Sys.Date())`"
lang: "en"
output: 
  html_document:
    includes:
      before_body: "header_logo.html"
    theme: cerulean
    css: "style.css"
    toc: true
    toc_depth: 3
    toc_float: true
    embed-resources: true
    self-contained: true
  word_document:
    reference_docx: "template.docx"
  pdf_document:
    includes:
      in_header: "preamble.tex"
params: 
  data: NULL
  disease: "Pertussis"
  country: "Austria"
  number_of_weeks: 6
  algo: "farrington"
  strata: !r c("county_id", "community_id", "sex", "age_group") 
  interactive: false
  tables: true
  signal_results: NULL
  signals_agg: NULL

---


```{r, echo=FALSE}
knitr::opts_chunk$set(eval = TRUE, 
                      echo = FALSE, 
                      fig.align = 'center',
                      fig.width = 13,
                      fig.height = 5,
                      message = FALSE, 
                      warning = FALSE)
```


```{r, results='asis'}
res <- knitr::knit_child('SignalDetectionReport_body.Rmd', quiet = TRUE)
cat(res)
```


```{r, results='asis'}

for (strat in params$strata) {
  res <- knitr::knit_child('SignalDetectionReport_strata.Rmd', quiet = TRUE)
  cat(res, sep = '\n')
}
```





