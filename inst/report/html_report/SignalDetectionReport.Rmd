...
This report contains an overview of signals observed for the period `r head(records$yearweek, 1)` to `r tail(records$yearweek, 1)` for `r text_diseases` in `r params$country`. The signals were calculated using the `r params$method` algorithm`r ifelse(grepl("glm|farrington", params$method) && !is.null(params$alpha_upper), paste0(" with a p-value cutoff of ", params$alpha_upper, "."), paste0("."))` The following filters were applied in the application: `r paste0(ifelse(length(params$selected_filter_vars) != 0, paste(params$selected_filter_vars, collapse = ", "), "None"))`. `r ifelse(params$filter_outbreak_cases == TRUE && grepl("glm", params$method), paste0(" Only cases not associated with outbreaks are used for training and testing."), paste0(""))`
