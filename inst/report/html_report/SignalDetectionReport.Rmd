...
This report contains an overview of signals observed for the period `r head(records$yeartimeunit, 1)` to `r tail(records$yeartimeunit, 1)` for `r text_diseases` in `r params$country`. The signals were calculated using the `r params$method` algorithm.
...
maxsignals <- params$signals_agg %>% 
  ...
  dplyr::summarise(max_signals = n()*params$number_of_time_units) %>% 
  dplyr::mutate(
    category = dplyr::case_when(
      is.na(category)~"Unstratified", 
      .default = category
